...
if(!file.exists(sprintf("../windows/gdal2-%s/include/gdal/gdal.h", VERSION))){
  ...
  unlink("lib.zip")
  download.file(sprintf("https://github.com/rgeoda/libgeoda_static/archive/v0.0.1.zip", VERSION), "geodalib.zip", quiet = TRUE)
  unzip("geodalib.zip", exdir = "../windows")
  unlink("geodalib.zip")
  download.file(sprintf("https://github.com/rgeoda/wx_static/archive/v3.0.4.zip", VERSION), "wxlib.zip", quiet = TRUE)
