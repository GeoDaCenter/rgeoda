...
That's it! It should be very fast: rgeoda uses wkb, which is a binary representation of geometries, to exchange data between `sp` and `libgeoda` in memory. 
...
legend('bottomright', legend = lisa_labels, fill = lisa_color, border = "#eeeeee")
```
